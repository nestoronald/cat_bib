...
(function($)
{
    ...
    function fileTemplate(list, fileobj, settings)
    {
	    ...
	    var upload	= $('<a title="Cargar Archivo" class="ax-upload ax-button" />').click(function(){
	    	if(settings.enable) fileobj.sns();   
	    }).appendTo(toolbar).append('<span class="ax-upload-icon ax-icon"></span>');
	    
	    var remove	= $('<a title="Eliminar archivo" class="ax-remove ax-button" />').click(function(){
	    	if(settings.enable) fileobj.remove(fn);//Envia el nombre del archivo a eliminar para su posterior eliminación física mediante xajax_delete_file
    ...
    function ajaxList(list, obj, settings)
    {
	    ...
	    obj.remove = function(name_file)
	    {
	    	...
	    	this.tools.li.remove();
                //alert(name_file);
                xajax_delete_file(name_file);//Elimina físicamente el archivo
	    };
    }
   ...
   var globalSettings = 
    {
    	...
    	maxFiles:		9999,				//Número máximo de archivos se puede seleccionar / max number of files can be selected
    	allowExt:		[],		//array of allowed upload extesion, can be set also in php script
    	showSize:		'Mb',						//show size in Mb, Kb or bytes, or Gb
    	success:		function(fileName){},		//función que se activa cada vez que se carga un archivo / function that triggers every time a file is uploaded
    	finish:			function(arrFiles){},		//función que se activa cuando todos los archivos se cargan / function that triggers when all files are uploaded
    	error:			function(txt,fileName){},	//función que se activa si se produce un error durante la carga / function that triggers if an error occuors during upload
	...
	var methods =
	{
		init : function(options)
		{
    	    ...
    	    {	
				...
				//var fieldSet  	= $('<fieldset />').append('<legend class="ax-legend">Seleccionar Archivo</legend>').appendTo($this);
                                var fieldSet  	= $('<fieldset />').append('').appendTo($this);
				var browse_c  	= $('<a class="ax-browse-c ax-button" title="Agregar Archivo" />').appendTo(fieldSet);//Browse container
				browse_c.append('<span class="ax-plus-icon ax-icon"></span> <span>Agregar Archivo</span>');
				
				var browseFiles = $('<input type="file" class="ax-browse" name="ax-files[]" />').attr('multiple',isAjaxUpload).appendTo(browse_c);
				
			    var uploadFiles = $('<a class="ax-upload-all ax-button" title="Iniciar carga" />').appendTo(fieldSet);
			    uploadFiles.append('<span class="ax-upload-icon ax-icon"></span> <span>Iniciar carga</span>');
			    
			    //var removeFiles = $('<a class="ax-clear ax-button" title="Retirar todo" />').appendTo(fieldSet);
                            var removeFiles = $('').appendTo(fieldSet);
			    removeFiles.append('<span class="ax-clear-icon ax-icon"></span> <span>Retirar todo</span>');
